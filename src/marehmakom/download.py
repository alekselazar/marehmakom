...
def _download(file_id: str, dest: str, silent=False):
    ...
    if not os.path.exists(dest):
        ...
        gdown.download(url, dest, quiet=silent)
    return dest

def ensure_assets(cfg: Config | None = None, silent=True):
    cfg = cfg or Config()
    tok_path = os.path.join(cfg.cache_dir, TOKENIZER_NAME)
    ckpt_path = os.path.join(cfg.cache_dir, CHECKPOINT_NAME)
    _download(cfg.tokenizer_file_id, tok_path, silent=silent)
    _download(cfg.checkpoint_file_id, ckpt_path, silent=silent)
